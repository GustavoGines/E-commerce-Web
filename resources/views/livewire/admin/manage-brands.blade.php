...
new #[Layout('layouts.app')] class extends Component {
    ...
    {
        $this->validate([
            'brand_name' => 'required|string|max:255|unique:brands,name,' . $this->brand_id
        ]);
        ...
        session()->flash('message', 'Marca guardada exitosamente.');
    }
    ...
    public function deleteBrand($id)
    {
        ...
        if ($brand->products()->count() > 0) {
            session()->flash('error', 'No se puede eliminar la marca porque tiene productos.');
            return;
        }
        $brand->delete();
        $this->loadBrands();
...
<div class="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
    <div class="flex justify-between items-center mb-8">
        <div>
            <h1 class="text-3xl font-extrabold text-gray-900 dark:text-white tracking-tight">Gestión de Marcas</h1>
            <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">Agrega, edita o elimina las marcas de tu tienda.</p>
        </div>
        <button wire:click="createBrand" class="text-white font-bold py-2.5 px-6 rounded-full shadow-lg transition-all hover:opacity-90 flex items-center space-x-2" style="background-color: var(--color-primary); box-shadow: 0 4px 14px 0 var(--color-primary-glow);">
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900 dark:text-gray-100">{{ $brand->name }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-center text-[var(--color-primary)] font-bold">{{ $brand->products_count }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-right space-x-3">
                            <button wire:click="editBrand({{ $brand->id }})" class="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-300">Editar</button>
                            <button wire:click="deleteBrand({{ $brand->id }})" class="text-red-600 hover:text-red-900 dark:text-red-400 dark:hover:text-red-300">Eliminar</button>
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="3" class="px-6 py-8 text-center text-sm text-gray-500 dark:text-gray-400">No hay marcas registradas.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
            ...
            <div class="inline-block align-bottom bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full">
                <div class="px-8 pt-8 pb-4">
                    <h3 class="text-2xl leading-6 font-bold text-gray-900 dark:text-white mb-6 tracking-tight">
                        {{ $brand_id ? 'Editar Marca' : 'Nueva Marca' }}
                    </h3>
                    <form wire:submit="saveBrand">
                        <div class="mb-6">
                            ...
                            <input wire:model="brand_name" type="text" class="w-full py-3 px-4 bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-xl text-gray-900 dark:text-white focus:ring-2 focus:ring-[var(--color-primary)]">
