        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12 lg:gap-8">
            ...
            <div class="lg:col-span-1">
                ...
                <div>
                    ...
                    <ul class="space-y-3">
                        ...
                        @auth
                            ...
                            @else
                                <li><a href="{{ route('my-orders') }}" wire:navigate class="text-gray-500 dark:text-gray-400 hover:text-[var(--color-primary)] text-sm font-medium transition-colors">Mis Órdenes</a></li>
                                <li><a href="{{ route('profile') }}" wire:navigate class="text-gray-500 dark:text-gray-400 hover:text-[var(--color-primary)] text-sm font-medium transition-colors">Mi Perfil</a></li>
                            @endif
                        @else
                            <li><a href="{{ route('login') }}" wire:navigate class="text-gray-500 dark:text-gray-400 hover:text-[var(--color-primary)] text-sm font-medium transition-colors">Iniciar Sesión / Registro</a></li>
                        @endauth
                    </ul>
                </div>
                ...
                <div>
                    ...
                    <ul class="space-y-4 mb-6">
                        @if(isset($this->social['whatsapp']) && !empty($this->social['whatsapp']))
                            ...
                            @endphp
                            <li>
                                <a href="{{ $waLink }}" target="_blank" class="flex items-center gap-3 text-gray-500 hover:text-green-600 transition-colors group">
                                    <div class="w-8 h-8 rounded-full bg-gray-100 dark:bg-slate-800 group-hover:bg-green-100 dark:group-hover:bg-green-900/30 flex items-center justify-center transition-colors">
                                    ...
                                    <div>
                                        ...
                                        <span class="text-sm font-bold">{{ $this->social['whatsapp'] }}</span>
                                    </div>
                                </a>
                            </li>
                        @endif
                        <li>
                            <a href="mailto:jcgelectronicaoficial@gmail.com" class="flex items-center gap-3 text-gray-500 hover:text-[var(--color-primary)] transition-colors group">
                                <div class="w-8 h-8 rounded-full bg-gray-100 dark:bg-slate-800 group-hover:bg-red-50 dark:group-hover:bg-red-900/30 flex items-center justify-center transition-colors">
                ...
                @if((isset($this->social['instagram']) && !empty($this->social['instagram'])) || (isset($this->social['facebook']) && !empty($this->social['facebook'])) || (isset($this->social['tiktok']) && !empty($this->social['tiktok'])))
                    <div>
                        <h5 class="text-xs font-black text-gray-900 dark:text-white uppercase tracking-widest mb-4">Seguinos en redes</h5>
                        <div class="flex items-center gap-3">
                            @if(isset($this->social['instagram']) && !empty($this->social['instagram']))
                                <a href="{{ $this->social['instagram'] }}" target="_blank" title="Instagram" class="w-9 h-9 rounded-full bg-gray-100 dark:bg-slate-800 text-gray-500 hover:bg-pink-100 hover:text-pink-600 flex items-center justify-center transition-colors">
                            ...
                            @if(isset($this->social['facebook']) && !empty($this->social['facebook']))
                                <a href="{{ $this->social['facebook'] }}" target="_blank" title="Facebook" class="w-9 h-9 rounded-full bg-gray-100 dark:bg-slate-800 text-gray-500 hover:bg-blue-100 hover:text-blue-600 flex items-center justify-center transition-colors">
                                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.469h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.469h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                                </a>
                            @endif
                            @if(isset($this->social['tiktok']) && !empty($this->social['tiktok']))
                                <a href="{{ $this->social['tiktok'] }}" target="_blank" title="TikTok" class="w-9 h-9 rounded-full bg-gray-100 dark:bg-slate-800 text-gray-500 hover:bg-gray-200 hover:text-black dark:hover:text-black flex items-center justify-center transition-colors">
