...
<div class="{{ $compact ? 'mt-3' : 'mt-4' }} w-full">
    @if($product->stock > 0)
        <div class="flex items-center {{ $compact ? 'gap-2' : 'gap-4' }}">
            ...
            <div class="flex items-center border border-gray-200 dark:border-gray-700 rounded-xl bg-gray-50 dark:bg-gray-800/50 p-1 flex-shrink-0">
                <button wire:click="decrementQuantity" type="button" class="{{ $compact ? 'w-8 h-8' : 'w-10 h-10' }} flex items-center justify-center rounded-lg bg-white dark:bg-gray-700 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-600 shadow-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed" @if($quantity <= 1) disabled @endif>
                ...
                <span class="{{ $compact ? 'w-6 text-sm' : 'w-10' }} text-center font-bold text-gray-900 dark:text-white">{{ $quantity }}</span>
            ...
            <button
                ...
                class="flex-1 flex items-center justify-center {{ $compact ? 'py-2.5 px-2' : 'py-3.5 px-4' }} rounded-xl text-white font-bold tracking-wide transition-all shadow-md hover:shadow-lg hover:-translate-y-0.5 active:translate-y-0 disabled:opacity-70 disabled:cursor-not-allowed group"
                style="background-color: var(--color-primary); box-shadow: 0 4px 10px -2px var(--color-primary-glow);">
                <svg wire:loading.remove wire:target="addToCart" class="w-5 h-5 {{ $compact ? 'mr-1' : 'mr-2' }} transition-transform group-hover:scale-110" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
                </svg>
                <svg wire:loading wire:target="addToCart" class="animate-spin -ml-1 {{ $compact ? 'mr-1' : 'mr-3' }} h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
